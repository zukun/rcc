...
//
// Given a file with pyramids, computes only the self-similarities,
// returning the result in a KernelMatrix.
...
int main(int argc, char** argv) {
  ...
  for (int ii = 0; ii < (int)vec.size(); ++ii) {
    ...
    km.at(ii, ii) =
      PyramidMatcher::GetPyramidMatchSimilarity(*vec[ii], *vec[ii],
