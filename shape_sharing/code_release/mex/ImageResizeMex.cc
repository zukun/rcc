...
// rhs (input): 
// src_bw (1xn cell, each cell: 2d binary image)
// dst_bbox (4xn matrix, lx,rx,ty,dy)
// src_loc (2xn matrix)
// dst_loc (2xn matrix)
// out_size (1x2 vector)
// lhs (output):
...
// trans (4xn matrix, each column: sx,sy,tx,ty)
{
 	if ( nrhs != 5 ) { 
	...
	if ( nlhs < 2 || nlhs > 5) {
		mexPrintf("2, 3, 4 or 5 outputs are only allowed\n");
		return;
	}	
	
	// parameters
	size_t n_proj = mxGetNumberOfElements(prhs[0]);
	double* out_size = mxGetPr(prhs[4]);
	...
	if ( nlhs == 2) {
		...
		plhs[2] = mxCreateDoubleMatrix(1, n_proj, mxREAL);
		plhs[3] = mxCreateCellArray(1, dim);
        plhs[4] = mxCreateDoubleMatrix(4, n_proj, mxREAL);
    }

	// projection
	double* dst_bbox = mxGetPr(prhs[1]);
	double* src_loc = mxGetPr(prhs[2]);
    double* dst_loc = mxGetPr(prhs[3]);
	...
	for (size_t i=0 ; i < n_proj ; i++ ) {
		...
		// boundbox of src
		size_t src_lx = im_w-1;
		size_t src_rx = 0;
		size_t src_ty = im_h-1;
		size_t src_dy = 0;
		for (size_t x=0; x < im_w ; x++) {
			for (size_t y=0; y < im_h ; y++) {
				size_t idx = x*im_stride + y;
				if ( bw[idx] ) {
					if ( src_lx > x) src_lx = x;
					if ( src_rx < x) src_rx = x;
					if ( src_ty > y) src_ty = y;
					if ( src_dy < y) src_dy = y;
				}
			}
		}

		// boundbox of dst
		size_t dst_lx = dst_bbox[4*i]-1; //zero-based index
		size_t dst_rx = dst_bbox[4*i+1]-1;
		size_t dst_ty = dst_bbox[4*i+2]-1;
		size_t dst_dy = dst_bbox[4*i+3]-1;
        
        vector<size_t> proj_inds;
		vector<unsigned char> proj_bw(out_h*out_w,0);
        int proj_min_x(out_w-1);
		int proj_max_x(0);
		int proj_min_y(out_h-1);
		int proj_max_y(0);
		double sx = (dst_rx-dst_lx+1.0)/(src_rx-src_lx+1.0);
		double sy = (dst_dy-dst_ty+1.0)/(src_dy-src_ty+1.0);
		double tx = 0;
        double ty = 0;
        if ( sx > 0 && sy > 0 ) {
            tx = dst_loc[2*i] - sx*src_loc[2*i];
            ty = dst_loc[2*i+1] - sy*src_loc[2*i+1];

            // back-projection
            proj_min_x = max(0, (int)(sx*src_lx + tx + 0.5));
            proj_max_x = min((int)out_w-1, (int)(sx*src_rx + tx + 0.5));
            proj_min_y = max(0, (int)(sy*src_ty + ty + 0.5));
            proj_max_y = min((int)out_h-1, (int)(sy*src_dy + ty + 0.5));
            ...
            for ( int x=proj_min_x; x <= proj_max_x ; x++) {
                for (int y=proj_min_y; y <= proj_max_y ; y++) {
                    ...
                    int x_ori = (int)floor(((double)x-tx)/sx + 0.5);
                    int y_ori = (int)floor(((double)y-ty)/sy + 0.5);
		...
		if ( nlhs == 2 ) {
            ...
            double* trans = mxGetPr(plhs[4]);
            unsigned int trans_offset = 4*i;
            trans[trans_offset] = sx;
            trans[trans_offset+1] = sy;
            trans[trans_offset+2] = tx;
            trans[trans_offset+3] = ty;
