...
/* Jaechul added */
void SetDataCost(GCoptimization *MyGraph, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]);
void SetSmoothnessCost(GCoptimization *MyGraph, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]);
    ...
    )
{
    ...
    switch (mode) {
        ...
        case 'n': /* get the current labeling energy */
            ...
            break;
        case 'd': /* set DataCost */
            SetDataCost(MyGraph, nlhs, plhs, nrhs, prhs);
            break;
        case 'm': /* set SmoothnessCost */
            SetSmoothnessCost(MyGraph, nlhs, plhs, nrhs, prhs);
            break;
        default:
...
/**************************************************************************************/
/* set user defined DataCost to graph, Jaechul */
void SetDataCost(GCoptimization *MyGraph, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    /* we need exactly 3 input arguments: gch, mode, DataCost */
    if (nrhs != 3 ) 
        mexErrMsgIdAndTxt("GraphCut:SetDataCost","Wrong number of input arguments");
    if ( mxGetClassID(prhs[2]) != mxSINGLE_CLASS ) 
        mexErrMsgIdAndTxt("GraphCut:SetDataCost","DataCost argument is not of type float");
    if ( mxGetNumberOfElements(prhs[2]) != (MyGraph->GetNumPixels() * MyGraph->GetNumLabels()) )
        mexErrMsgIdAndTxt("GraphCut:SetDataCost","DataCost argument does not contains the right number of elements");
    
    /* verify only one output parameter */
    if (nlhs != 1)
        mexErrMsgIdAndTxt("GraphCut:SetDataCost","wrong number of output parameters");
    
    Graph::captype *DataCost = (Graph::captype*)mxGetData(prhs[2]);
    MyGraph->modifyData(DataCost);
}

/**************************************************************************************/
/* set user defined SmoothnessCost to graph, Jaechul */
void SetSmoothnessCost(GCoptimization *MyGraph, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    /* we need exactly 3 input arguments: gch, mode, DataCost */
    if (nrhs != 3 ) 
        mexErrMsgIdAndTxt("GraphCut:SetSmoothnessCost","Wrong number of input arguments");
    if ( mxGetClassID(prhs[2]) != mxSINGLE_CLASS ) 
        mexErrMsgIdAndTxt("GraphCut:SetSmoothnessCost","SmoothnessCost argument is not of type float");
    if ( mxGetNumberOfElements(prhs[2]) != (MyGraph->GetNumLabels() * MyGraph->GetNumLabels()) )
        mexErrMsgIdAndTxt("GraphCut:SetSmoothnessCost","SmoothnessCost argument does not contains the right number of elements");
    
    /* verify only one output parameter */
    if (nlhs != 1)
        mexErrMsgIdAndTxt("GraphCut:SetSmoothnessCost","wrong number of output parameters");
    
    Graph::captype *SmoothnessCost = (Graph::captype*)mxGetData(prhs[2]);
    MyGraph->modifySmoothness(SmoothnessCost);
}
