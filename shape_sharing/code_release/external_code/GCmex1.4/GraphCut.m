...
switch lower(mode)
    ...
    case {'sd', 'setdata'}
        if nargout ~= 1
            error('GraphCut:SetDatCost: Too many outputs');
        end
        
        [gch] = SetDataCost(varargin{:});
        
    case {'ss', 'setsmoothness'}
        if nargout ~= 1
            error('GraphCut:SetSmoothnessCost: Too many outputs');
        end
        
        [gch] = SetSmoothnessCost(varargin{:});      
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [gch] = SetDataCost(varargin)
% usage [gch] = SetDataCost(gch, DataCost)
% added by Jaechul
if nargin ~= 2
    error('GraphCut:SetDataCost: wrong number of inputs');
end
gch = varargin{1};
DataCost = varargin{2};

if ~strcmp(class(DataCost), 'single')
    DataCost = single(DataCost);
end
DataCost = permute(DataCost,[3 2 1]);
DataCost = DataCost(:)';
[gch] = GraphCutMex(gch, 'd', DataCost);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [gch] = SetSmoothnessCost(varargin)
% usage [gch] = SetSmoothnessCost(gch, DataCost)
% added by Jaechul
if nargin ~= 2
    error('GraphCut:SetSmoothnessCost: wrong number of inputs');
end
gch = varargin{1};
SmoothnessCost = varargin{2};

if ~strcmp(class(SmoothnessCost), 'single')
    SmoothnessCost = single(SmoothnessCost);
end
SmoothnessCost = SmoothnessCost(:)';
[gch] = GraphCutMex(gch, 'm', SmoothnessCost);
