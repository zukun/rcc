function [BPLR, desc] = demo(img_file, rsz, order_k, euc_f, min_nseg, max_nseg, grid_space, min_elem_scale, magnif, phog_L, n_angle_bin, n_length_bin)
...
% desc.feat_vecs = color_hist;
% desc.feat_scales = feat_scales;
% desc.feat_centers = feat_centers;
% desc_t = toc;

disp([num2str(bplr_t) ' seconds for BPLR extraction']);
disp([num2str(desc_t) ' seconds for descriptor']);
disp([num2str(bplr_t + desc_t) ' seconds in total']);
